...
#include "r2_robot/pid_velocity_controller.h"
...
public:
	void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
		...
		this->updateConnection =
				event::Events::ConnectWorldUpdateBegin(std::bind(&R2ArmPlugin::OnUpdate, this));
		...
		if (!_sdf->HasElement("update_rate")) {
			...
			this->update_rate_ = common::Time(0, common::Time::SecToNano(u_rate)).Double();
		}
		...
		ros::SubscribeOptions so = ros::SubscribeOptions::create<geometry_msgs::Point>(
				"/arm/set_pos", 1, boost::bind(&R2ArmPlugin::cmdPosCallback, this, _1), ros::VoidPtr(),
				&this->rosQueue);
		this->rosSub = this->rosNode->subscribe(so);

		// Create a named topic, and subscribe to it.
		ros::SubscribeOptions so_pid = ros::SubscribeOptions::create<std_msgs::Float32MultiArray>(
				"/arm/set_pid", 1, boost::bind(&R2ArmPlugin::cmdPIDCallback, this, _1), ros::VoidPtr(),
				&this->rosQueue);
		this->rosSub_pid = this->rosNode->subscribe(so_pid);

		// Spin up the queue helper thread.
		this->rosQueueThread = std::thread(std::bind(&R2ArmPlugin::QueueThread, this));
		...
		this->joint_state_publisher_ =
				this->rosNode->advertise<sensor_msgs::JointState>("/arm/joint_state", 1000);

		if (!this->pid_controller_set) {
			// setup pid controller

			this->arm_position_controller_.init(&this->arm_encoder_pos_, 0.8, 0.00002, 13, 0.00002, 1.8);
			this->pos_cmd_ = 0;
...
private:
	...
	void cmdPosCallback(const geometry_msgs::Point::ConstPtr &cmd_msg) {
		// Configure so that the position can be any value between 0 (fully retracted) and 0.375 (fully
		// extended).
	...
	void UpdateArmEncoder() {
		this->step_time_ = (this->robot_->GetWorld()->SimTime() - this->last_encoder_update_).Double();
		this->last_encoder_update_ = this->robot_->GetWorld()->SimTime();

		this->arm_encoder_vel_ =
				(this->arm_encoder_pos_ - this->joint_arm_->Position(0)) / this->step_time_;
