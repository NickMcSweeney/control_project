...
namespace gazebo {
  class R2D2ArmPlugin : public ModelPlugin {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) {
    	printf("Hello World!\n");
			...
			this->updateConnection = event::Events::ConnectWorldUpdateBegin(std::bind(&R2D2ArmPlugin::OnUpdate, this));
      ...
      // Create a named topic, and subscribe to it.
			ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float32>(
                    "/r2d2/vel_cmd",
                    1,
                    boost::bind(&R2D2ArmPlugin::OnRosMsg, this, _1),
                    ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);

      // Spin up the queue helper thread.
      this->rosQueueThread = std::thread(std::bind(&R2D2ArmPlugin::QueueThread, this));
  ...
  GZ_REGISTER_MODEL_PLUGIN(R2D2ArmPlugin)
}
