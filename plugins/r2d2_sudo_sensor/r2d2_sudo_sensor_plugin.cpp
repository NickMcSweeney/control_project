...
class R2D2SudoSensorPlugin : public ModelPlugin {
public:
  void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    this->rosQueueThread =
        std::thread(std::bind(&R2D2SudoSensorPlugin::QueueThread, this));
    ...
    this->object_pos_publisher_ =
        this->rosNode->advertise<geometry_msgs::Point>("/sudo_sensor/pos",
                                                          1000);
...
private:
  // publisher for r2d2's sudo sensor target object state.
...
GZ_REGISTER_MODEL_PLUGIN(R2D2SudoSensorPlugin)
}
