...
public:
	void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
		...
		this->updateConnection =
				event::Events::ConnectWorldUpdateBegin(std::bind(&PsudoSensorPlugin::OnUpdate, this));

		this->object_link_ = this->object_->GetLink("obj_body");
		...
		this->rosQueueThread = std::thread(std::bind(&PsudoSensorPlugin::QueueThread, this));
		...
		this->object_pos_publisher_ =
				this->rosNode->advertise<geometry_msgs::Point>("/psudo_sensor/pos", 1000);
		this->object_vel_publisher_ =
				this->rosNode->advertise<geometry_msgs::Twist>("/psudo_sensor/vel", 1000);
		this->object_effort_publisher_ =
				this->rosNode->advertise<geometry_msgs::Vector3>("/psudo_sensor/effort", 1000);
	}
...
private:
	...
	void UpdateObjectState() {
		// set time increment;
		this->step_time_ = (this->object_->GetWorld()->SimTime() - this->last_encoder_update_).Double();
		this->last_encoder_update_ = this->object_->GetWorld()->SimTime();
		// set current state;
		double pos[3] = {this->object_link_->WorldPose().Pos().X(),
										 this->object_link_->WorldPose().Pos().Y(),
										 this->object_link_->WorldPose().Pos().Z()};
