...
class R2GripperPlugin : public ModelPlugin {
public:
  void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    this->updateConnection = event::Events::ConnectWorldUpdateBegin(
        std::bind(&R2GripperPlugin::OnUpdate, this));
    ...
    ros::SubscribeOptions so_pos =
        ros::SubscribeOptions::create<geometry_msgs::Point>(
            "/gripper/set_pos", 1,
            boost::bind(&R2GripperPlugin::cmdPosCallback, this, _1),
    ...
    ros::SubscribeOptions so_force =
        ros::SubscribeOptions::create<std_msgs::Float32>(
            "/gripper/set_force", 1,
            boost::bind(&R2GripperPlugin::cmdForceCallback, this, _1),
            ros::VoidPtr(), &this->rosQueue);
    ...
    ros::SubscribeOptions so_pid =
        ros::SubscribeOptions::create<std_msgs::Float32MultiArray>(
            "/gripper/set_pid", 1,
            boost::bind(&R2GripperPlugin::cmdPIDCallback, this, _1),
            ros::VoidPtr(), &this->rosQueue);
    this->rosSub_pid = this->rosNode->subscribe(so_pid);

    // Spin up the queue helper thread.
    this->rosQueueThread =
        std::thread(std::bind(&R2GripperPlugin::QueueThread, this));
...
GZ_REGISTER_MODEL_PLUGIN(R2GripperPlugin)
}
